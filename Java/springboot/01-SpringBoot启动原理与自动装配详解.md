...
> 建议按官方命名规范：自动配置类后缀 `AutoConfiguration`，并排在前（模块化后见 [03-SpringBoot模块化详解](03-SpringBoot模块化详解.md)）。完整实战（双模块拆分/元数据/测试/示例）见 [04-SpringBoot自定义Starter详解](04-SpringBoot自定义Starter详解.md)。
