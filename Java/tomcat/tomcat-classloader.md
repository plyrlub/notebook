...
aliases: [Tomcat 类加载机制剖析]
---

# Tomcat 类加载机制剖析

> 本文是 Tomcat 学习笔记第 5 章（wolai 转存整理）。wolai 原文仅列了 JDK 三大类加载器，未展开 Tomcat 特有的类加载体系。
> **深度展开见 [tomcat-classloader-deepdive](tomcat-classloader-deepdive.md)**（WebAppClassLoader 与双亲委派的打破、类隔离、热部署原理，已单独成篇）。
> 关联笔记：[tomcat-overview](tomcat-overview.md)、**Java 类加载机制与双亲委派详解**（见知识库）
...
> 原文到此为止。wolai 笔记对类加载只做了引出，**完整机制（为什么打破双亲委派、WebAppClassLoader 加载顺序、java.* 保护、热部署原理、易错点、面试 Q&A）已整理在 [tomcat-classloader-deepdive](tomcat-classloader-deepdive.md)**，本节仅列体系骨架：
...
**核心结论**（详见 [tomcat-classloader-deepdive](tomcat-classloader-deepdive.md)）：
...
*来源：wolai 笔记转存（Apache Tomcat 学习笔记第 5 章），深度内容并入 [tomcat-classloader-deepdive](tomcat-classloader-deepdive.md)，2026-08-08 整理*
