...
- 关联笔记：[01-代码混淆详解](../../../通用技术/软件保护/01-代码混淆详解.md)（混淆会重命名反射目标类/方法，反射场景必须 keep）、[Java注解机制详解](Java注解机制详解.md)（反射读取注解）、[Java Agent与字节码增强详解](Java Agent与字节码增强详解.md)（运行期动态机制对比）
