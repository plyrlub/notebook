...
---
- 关联笔记：[01-代码混淆详解](../../../通用技术/软件保护/01-代码混淆详解.md)（SPI 的 META-INF/services 写全限定类名，混淆重命名后服务发现失败，实现类需 keep）、[Java反射详解](Java反射详解.md)（SPI 加载本质是反射实例化）、[Java注解机制详解](Java注解机制详解.md)（APT 与 SPI 的异同）
