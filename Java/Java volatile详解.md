...
- 线程与 synchronized 基本用法（本系列见 **01-Java线程池原理与参数详解**（见知识库））
- 理解「缓存行」「CPU 缓存」概念即可，MESI/屏障细节本文从零讲
- 相关阅读：**Java代理详解**（见知识库）（volatile 常用于 DCL 单例的安全发布）
...
- **Java-PriorityQueue详解**（见知识库） — 并发容器/同步器系列（volatile 的可见性是其基础）
- **Java代理详解**（见知识库） — DCL 单例之外的线程安全发布场景
...
---
*来源：Hermes 会话整理（2026-08-06，含 JMM/硬件层完整链路、MESI、四种屏障、假共享；2026-08-09 按 3.3~3.6 新模板改造 + 可见性 Demo 实测复现）*
